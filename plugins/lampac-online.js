(function() {
    'use strict';
    const localhost = `${window.location.protocol}//${window.location.host}:9118`
          ...
          var fin = function fin(call) {
            ...
            network.silent(account(url), function(json) {
              ...
              json.online.forEach(function(j) {
                var name = balanserName(j);
                console.log("LAMPAC loading", j,", lampac-online.js", j.url, localhost, j.url.replace("http://localhost:9118", localhost));
                sources[name] = {
                  // PATCH START
                  url: j.url.replace("http://localhost:9118", localhost),
                  // PATCH END
                  name: j.name,
                  show: typeof j.show == 'undefined' ? true : j.show
                };
    ...
    function startPlugin() {
        console.log("[Lampac Online]", "Starting Lampac Online plugin", "");
