...
function withCORS(headers, request) {
  // PATCH START
  console.log(request.rawHeaders, request.headers);
  const origin = request.headers['origin']
  const ref = request.headers['referer'];
  let result = origin || ref;
  console.log(origin, ref, result)
  if (result && !result.startsWith("https://")) result = "http://" + result.split("http://")[1].split("/")[0]
  console.log(result)
  headers['access-control-allow-origin'] = result || '*';
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  console.log(proxyRes.headers['set-cookie'] && proxyRes.headers['set-cookie'].length)
  if (proxyRes.headers['set-cookie'] && proxyRes.headers['set-cookie'].length) proxyRes.headers['set-cookie'][0] += '; SameSite=None';
  console.log(proxyRes.headers)
  console.log("!!!=========")
  return true;
}
